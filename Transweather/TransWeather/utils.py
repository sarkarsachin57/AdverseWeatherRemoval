...
def validation_val(net, val_data_loader, device, exp_name, category, save_tag=False,results_path='./results'):
...
# --- Calculate the average PSNR --- #
        ...
        if save_tag:
            # print()
            save_image(pred_image, imgid, exp_name,category,results_path)

    avr_psnr = sum(psnr_list) / len(psnr_list)
    avr_ssim = sum(ssim_list) / len(ssim_list)
    return avr_psnr, avr_ssim

def save_image(pred_image, image_name, exp_name, category, results_path):
    ...
    for ind in range(batch_num):
        ...
        print(image_name_1)
        utils.save_image(pred_image_images[ind],os.path.join(results_path,image_name_1))
